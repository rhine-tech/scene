...
func (s *CtxProxy) WithSceneContext(ctx scene.Context) authentication.IAccessTokenService {
...
func (a *accessTokenServiceCtx) Create(userId, name string, expireAt int64) (authentication.AccessToken, error) {
	...
	if !ok {
		a.logger.WarnW("permission denied: no permission context found", "operation", "CreateToken")
		return authentication.AccessToken{}, permission.ErrPermissionDenied
	}

	if !pctx.HasPermission(authentication.PermTokenCreate) {
		a.logger.WarnW("permission denied: missing required permission", "operation", "CreateToken", "required_perm", authentication.PermTokenCreate)
		return authentication.AccessToken{}, permission.ErrPermissionDenied
	}
	...
	if !pctx.HasPermission(authentication.PermAdmin) {
		...
		if !ok {
			a.logger.WarnW("permission denied: user not logged in for self-service action", "operation", "CreateToken", "target_user", userId)
			return authentication.AccessToken{}, authentication.ErrNotLogin
		}
		if actx.UserID != userId {
			a.logger.WarnW("permission denied: user attempting to create token for another user", "operation", "CreateToken", "requesting_user", actx.UserID, "target_user", userId)
			return authentication.AccessToken{}, permission.ErrPermissionDenied
		}
	}

	a.logger.DebugW("permission granted", "operation", "CreateToken", "target_user", userId)
...
func (a *accessTokenServiceCtx) ListByUser(userId string, offset, limit int64) (result model.PaginationResult[authentication.AccessToken], err error) {
	pctx, ok := permission.GetPermContext(a.ctx)
	if !ok {
		a.logger.WarnW("permission denied: no permission context found", "operation", "ListTokensByUser")
		return result, permission.ErrPermissionDenied
	}

	if !pctx.HasPermission(authentication.PermTokenList) {
		a.logger.WarnW("permission denied: missing required permission", "operation", "ListTokensByUser", "required_perm", authentication.PermTokenList)
		return result, permission.ErrPermissionDenied
	}
	...
	if !pctx.HasPermission(authentication.PermAdmin) {
		actx, ok := authentication.GetAuthContext(a.ctx)
		if !ok {
			a.logger.WarnW("permission denied: user not logged in for self-service action", "operation", "ListTokensByUser", "target_user", userId)
			return result, authentication.ErrNotLogin
		}
		if actx.UserID != userId {
			a.logger.WarnW("permission denied: user attempting to list tokens for another user", "operation", "ListTokensByUser", "requesting_user", actx.UserID, "target_user", userId)
			return result, permission.ErrPermissionDenied
		}
	}

	a.logger.DebugW("permission granted", "operation", "ListTokensByUser", "target_user", userId)
...
func (a *accessTokenServiceCtx) Delete(token string) error {
	pctx, ok := permission.GetPermContext(a.ctx)
	if !ok {
		a.logger.WarnW("permission denied: no permission context found", "operation", "DeleteToken")
		return permission.ErrPermissionDenied
	}

	if !pctx.HasPermission(authentication.PermTokenDelete) {
		a.logger.WarnW("permission denied: missing required permission", "operation", "DeleteToken", "required_perm", authentication.PermTokenDelete)
		return permission.ErrPermissionDenied
	}
	...
	if !pctx.HasPermission(authentication.PermAdmin) {
		actx, ok := authentication.GetAuthContext(a.ctx)
		if !ok {
			a.logger.WarnW("permission denied: user not logged in for self-service action", "operation", "DeleteToken")
		...
		if err != nil {
			...
			a.logger.InfoW("token validation failed during delete check", "operation", "DeleteToken", "requesting_user", actx.UserID, "error", err)
			return err
		}
		// Finally, check ownership.
		if actx.UserID != ownerId || !actx.IsLogin() {
			a.logger.ErrorW("permission denied: user attempted to delete another user's token", "operation", "DeleteToken", "requesting_user", actx.UserID, "token_owner", ownerId)
			return permission.ErrPermissionDenied
		}
	}

	a.logger.DebugW("permission granted", "operation", "DeleteToken")
