...
func (f *fakeAuthCache) Set(ctx context.Context, key string, value []byte, ttl time.Duration) error {
	return f.SetWithTags(ctx, key, value, ttl)
}

func (f *fakeAuthCache) SetWithTags(_ context.Context, key string, value []byte, _ time.Duration, tags ...string) error {
